...
public:
  ...
  __host__ __device__ bool near_zero() const {
    ...
    auto s = 1e-8f;
    return (std::fabs(e[0]) < s) && (std::fabs(e[2]) < s) && (std::fabs(e[2]) < s);
  }
...
__host__ inline vec3 random_unit_vector() {
  while (true) {
    ...
    // Floating-point abstraction leak changed from 1e-160 for double to 1e-40 for float
...
__host__ inline vec3 random_on_hemisphere(const vec3& normal) {
...
__host__ inline vec3 refract(const vec3& uv, const vec3& n, float etai_over_etat) {
